#!/usr/bin/env python3
"""
Upload processed chunks to Supabase with embeddings
...
def upload_to_supabase():
    ...
    """
    print("=" * 70)
    print("🚀 Phase 3: Upload to Supabase with Embeddings")
    print("=" * 70)
    print()
    ...
    print(f"   Cost estimate: ~${len(chunks) * 200 / 1_000_000 * 0.02:.4f}")
    print()
    
    generator = EmbeddingGenerator()
    start_time = datetime.now()
    
    embedded_chunks = generator.embed_chunks(chunks, batch_size=100)
    ...
    # Check if table exists (try to get count)
    try:
        ...
        print("\nMake sure you've created the table. Run this to see the SQL:")
        print("  python -m app.rag.vector_store")
        return False
    ...
    if stats['failed'] == 0:
        ...
        print("  python scripts/test_supabase_search.py")
        return True
    else:
